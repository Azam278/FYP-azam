            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Question Paper</h1>
                        <a onclick="document.getElementById('id04').style.display='block'" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">Create New</a>
                    </div>

                    <div id="id04" class="tablelist">
                        <h2>Question Paper Subject</h2>
                        <form action="addquestionlist.php" method="post" class="needs-validation" novalidate>
                            <table id="tableJPU">
                                <tr>
                                    <td>Course Name</td>
                                    <td>:</td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                            // Query to fetch Course ID (assuming it's auto-generated)
                                            $courseQuery = "SELECT CourseID FROM course"; // Add conditions to select the desired Course ID
                                            $courseResult = mysqli_query($conn, $courseQuery);

                                            // Check if the Course ID is found
                                            if ($courseRow = mysqli_fetch_assoc($courseResult)) {
                                                $courseID = $courseRow['CourseID'];

                                                // Query to fetch Assessment Types based on Course ID
                                                $assessmentQuery = "SELECT AssessmentID, AssessmentType FROM assessment WHERE CourseID = $courseID";
                                                $assessmentResult = mysqli_query($conn, $assessmentQuery);
                                            }
                                        ?>

                                        <select class="form-control" name="questiontype" id="questiontype" aria-label="Default select example" required>
                                            <option value="">Select Question</option>

                                            <?php
                                                // Display Assessment Types as options
                                                if (isset($assessmentResult)) {
                                                    while ($row = mysqli_fetch_assoc($assessmentResult)) {
                                                        echo '<option value="' . $row['AssessmentID'] . '">' . $row['AssessmentType'] . '</option>';
                                                    }
                                                }
                                            ?>
                                        </select>
                                ...
                                <tr>
                                    ...
                                    <td>:</td>
                                    <td>
                                        <input class="form-control" type="text" name="questiondesc" placeholder="Enter Description" pattern="^[A-Za-z\s]+$" required>
                                        <div class="invalid-feedback">
                            ...
                            <button type="button" onclick="document.getElementById('id04').style.display='none'" class="btn-cancel">Cancel</button>               
                        </form>
                    </div>
                            ...
                            if (isset($_SESSION['userid'])) {
                                ...
                                if ($result) {
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        echo "<tr><th>No.</th><th>Course Name</th><th>Question Type</th><th class='actionCol' colspan='2'>Action</th></tr>";
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            $description = $row['AssessmentType'];
                                            ...
                                            echo '<td class="text-center"><a class="btn btn-primary" href="qpdetails.php?action=add&course_id=' . $courseID . '"><i class="bi bi-plus-lg"> Add</i></a></td>';
                                            // Add the edit button
                                            echo '<td class="text-center"><a class="btn btn-success" href="qpdetails.php?action=edit&course_id=' . $courseID . '"><i class="bi bi-pen"> Edit</i></a></td>';
                                            echo "</tr>";
