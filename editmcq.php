<?php
    ...
    $queryOptions = "SELECT ChoiceID, ChoiceText FROM choice WHERE QPLID = '$qplID'";
    ...
    if ($resultOptions) {
        ...
        while ($rowOptions = mysqli_fetch_assoc($resultOptions)) {
            $options[] = array(
                'ChoiceID' => $rowOptions['ChoiceID'],
                'ChoiceText' => $rowOptions['ChoiceText']
            );
        }
    } else {
        // Error in the SQL query
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $updatedMarkText = $_POST['mark'];
        $updatedstatus = "pending";
        $updatedOptions = $_POST['options'];

        // Update the question details in the database
        $updateQuery = "UPDATE question_paper_list SET Question = '$updatedQuestionText', Mark = '$updatedMarkText', QPLStatus = '$updatedstatus' WHERE QPLID = '$qplID'";
        $result = mysqli_query($conn, $updateQuery);

        if ($result) {

            // Update the options in the database
            $updatedOptionIDs = $_POST['option_ids'];
            $updatedOptions = $_POST['options'];

            foreach ($updatedOptions as $index => $option) {
                $optionID = $updatedOptionIDs[$index];
                $updateOptionQuery = "UPDATE choice SET ChoiceText = '$option' WHERE ChoiceID = '$optionID'";
                $result = mysqli_query($conn, $updateOptionQuery);
                ...
                // Update the answer according to the ChoiceID
                $updateAnswerQuery = "UPDATE answer SET Ans_Schema = '$option' WHERE ChoiceID = '$optionID' AND QPLID = '$qplID'";
                $result = mysqli_query($conn, $updateAnswerQuery);
            
                if (!$result) {
                    // Error in query
                    echo 'Error in updating answer ' . ($index+1) . ': ' . mysqli_error($conn);
                    exit();
                }
            }           
            ...
            // Update failed
            // $errorMessage = "Error updating question details: " . mysqli_error($conn);
            // echo $errorMessage;
            echo "<script>
            window.onload = function() {
                Swal.fire({
                    ...
                    <div class="row">
			            ...
			            <div class="editQuestionForm">
                            <div class="container">
                                ...
                                <form action="editmcq.php?qpl_id=<?php echo $qplID; ?>&course_id=<?php echo $courseID; ?>" method="POST">
                                    <table>
                                        <tr>
                                            <td>Question</td>
                                            <td>:</td>
                                            <td>
                                            <textarea class='form-control' style="height: 50px; width: 10%;" name="question" id="question"><?php echo $questionText; ?></textarea>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Mark</td>
                                            <td>:</td>
                                            <td>
                                                <input class='form-control' type="text" name="mark" value="<?php echo $markText; ?>">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Question Type</td>
                                        ...
                                        <tr>
                                            ...
                                            <td valign="top">
                                                <?php foreach ($options as $option) { ?>
                                                    <input type="hidden" name="option_ids[]" value="<?php echo $option['ChoiceID']; ?>">
                                                    <input type="text" name="options[]" value="<?php echo $option['ChoiceText']; ?>">
                                                    <br>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                    ...
                                    <div class="button-container">
                                        ...
                                        <button class="btn-edit" value="submit">Save</a></button>
                                    </div>
                                </form>
                            </div>
                        </div>
