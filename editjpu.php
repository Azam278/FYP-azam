<?php
    ...
    $queryMarks = "SELECT t.TID, dt.DTID, dt.DT_Level, mrk.MarkID, mrk.Mark
                FROM mark mrk
                INNER JOIN topic t ON t.TID = mrk.TID
    ...
    if ($resultMarks) {
        $options = array();
        while ($rowOptions = mysqli_fetch_assoc($resultMarks)) {
            $options[] = array(
                'DTID' => $rowOptions['DTID'],
                'DT_Level' => $rowOptions['DT_Level'],
                'MarkID' => $rowOptions['MarkID'],
                'Mark' => $rowOptions['Mark']
            );
        }
    } else {
        // Error in the SQL query
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $updatedTopicName = $_POST['topicname'];
        $updatedMarks = $_POST['options'];

        $updateQuery = "UPDATE topic SET T_Name = '$updatedTopicName' WHERE TID = '$topicID'";
        $result = mysqli_query($conn, $updateQuery);

        if ($result) {
            $updatedMarkIDs = $_POST['option_ids'];
            $updatedMarks = $_POST['options'];

            foreach ($updatedMarks as $index => $option) {
                $optionID = $updatedMarkIDs[$index];
                $updateMarkQuery = "UPDATE mark SET Mark = '$option' WHERE MarkID = '$optionID'";
                $result = mysqli_query($conn, $updateMarkQuery);
            ...
            window.onload = function() {
                Swal.fire({
                    ...
                    window.location.href = 'jpu.php?action=edit&course_id=" . $courseID . "';
                });
            }
            </script>";
        } else {
                    ...
                    <div class="row">
			            ...
			            <div class="editForm">
                            <div class="container">
                                <center>
                                    <h2>Edit JPU Item</h2>                                    
                                    <form action="editjpu.php?course_id=<?php echo $courseID; ?>&topic_id=<?php echo $topicID; ?>" method="post">
                                        <table class="table">
                                            <tr>
                                                <td>Topic</td>
                                                <td>:</td>
                                                <td>
                                                    ...
                                                    <input type="text" name="topicname" value="<?php echo $topicText; ?>">
                                                </td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                <td valign="top">
                                                    <?php foreach ($options as $option) { ?>
                                                        <label><?php echo $option['DT_Level']; ?></label>
                                                        <br>
                                                    <?php } ?>
                                                </td>
                                                <td valign="top">:</td>
                                                <td valign="top">
                                                    <?php foreach ($options as $option) { ?>
                                                        <input type="hidden" name="option_ids[]" value="<?php echo $option['MarkID']; ?>">
                                                        <input type="text" name="options[]" value="<?php echo $option['Mark']; ?>">
                                                        <br>
                                                    <?php } ?>
                                                </td>
                                            </tr>
                                        ...
                                        <div class="button-container">
                                            <a onclick="backToJPU('<?php echo $courseID; ?>')" type="button" class="btn-cancel">Cancel</a>
                                            <button type="submit" class="btn-edit" value="submit">Save</button>
                                        </div>                                        
                                    </form>
                                </center>
                            </div>
